...
mkdir -p "$TOOLS" "$ANDROID_HOME/cmdline-tools" "$GODOT_HOME" build build_support
...
if [ ! -x "$GODOT_HOME/godot" ]; then
  ...
  curl -fL "https://github.com/godotengine/godot-builds/releases/download/${GODOT_VERSION}-stable/Godot_v${GODOT_VERSION}-stable_linux.x86_64.zip" -o "$TOOLS/godot.zip"
...
if [ ! -f "$TEMPLATE_DIR/android_debug.apk" ]; then
  ...
  curl -fL "https://github.com/godotengine/godot-builds/releases/download/${GODOT_VERSION}-stable/Godot_v${GODOT_VERSION}-stable_export_templates.tpz" -o "$TOOLS/templates.tpz"
  rm -rf "$TOOLS/templates"
  mkdir -p "$TOOLS/templates"
  unzip -qo "$TOOLS/templates.tpz" -d "$TOOLS/templates"
  mkdir -p "$TEMPLATE_DIR"
  cp -a "$TOOLS/templates/templates/." "$TEMPLATE_DIR/"
fi
...
"$SDKMANAGER" --sdk_root="$ANDROID_HOME" "platform-tools" "platforms;android-36" "build-tools;${BUILD_TOOLS_VERSION}"
...
DEBUG_KEYSTORE="$ROOT/build_support/debug.keystore"
...
"$GODOT_HOME/godot" --headless --editor --path "$ROOT" --import --quit
...
"$ANDROID_HOME/build-tools/${BUILD_TOOLS_VERSION}/zipalign" -c -P 16 4 build/World-Defense.apk
"$ANDROID_HOME/build-tools/${BUILD_TOOLS_VERSION}/apksigner" verify --verbose --print-certs build/World-Defense.apk
"$ANDROID_HOME/build-tools/${BUILD_TOOLS_VERSION}/aapt" dump badging build/World-Defense.apk | tee build/BADGING.txt
