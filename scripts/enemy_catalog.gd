...
static func stage_pool(stage:int)->Array:
 var p=["assault","heavy"]
 if stage>=8:p+=["fast_armor"]
 if stage>=18:p+=["heavy_tank","anti_tank"]
 if stage>=30:p+=["aa_vehicle","helicopter"]
 if stage>=45:p+=["sam","artillery"]
 if stage>=70:p+=["fighter","support"]
 if stage>=95:p+=["bomber","ew"]
 return p
