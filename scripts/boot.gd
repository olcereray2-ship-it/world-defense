...
func _ready():
 ...
 LocalizationManager.apply(OS.get_locale())
...
func _start_qa():
 var err=get_tree().change_scene_to_file("res://scenes/QARunner.tscn")
 if err!=OK:push_error("WORLD_DEFENSE_QA_CHANGE_FAILED:%s"%err)

func _process(delta):
 if FileAccess.file_exists("user://qa.flag"):return
 elapsed+=delta
 if elapsed<0.65:
  var t=elapsed/0.65
  title.modulate.a=t
